...
```{r,echo=FALSE}
table(cut(subset(dados, contb_receipt_amt < 0)$contb_receipt_amt, breaks = c(-10000,-5000,-1000,-500,-100,0)))

paste( "Quantidade de valores negativos: ",length(subset(dados, contb_receipt_amt < 0)$contb_receipt_amt))
```
...
Uma nova analise com mais intervalos (abaixo) e comecando a partir de 75% temos que **95%** das doacoes chegam no maxximo a $400 e esse valor pode ser utilizado como limite nas visualizacoes futuras para facilitar o entendimento do grafico.
```{r,echo=FALSE}
quantile(subset(dados, contb_receipt_amt >= 0)$contb_receipt_amt, probs = seq(.75,1,0.05))
```

##Distribuicao das contribuicoes
Usando o limite de **$400** e criando um histograma vemos que a maioria das doacoes se encontram dentro de um pequeno intervalo de **0-100** dÛlares. Como os dados estao muito proximos uma transformacao de coordenadas pode ser util.
```{r,echo=FALSE}
...
Transformando a escala para **log10** temos uma distribuicao que mostra como os valores das doacoes sao concentrados e podemos visualizar 100% dos dados (anterior foi restrita a $400). Podemos ver que uma quantidade de significativa de contribuicoes ocorre ate **1300** dolares o que nos deixa da um intervalo de trabalho para as proximas analises e deixa os demais valores como **outliers**.
```{r transformando Log10,echo=FALSE}
ggplot(data = subset(dados, contb_receipt_amt > 0 ), aes(x = contb_receipt_amt))+
...
Agora que ja entendemos a distribuicao das doacoes e nos familiarzamos com os dados, podemos analisar para onde estas estao sendo direcionadas.

Vemos que Clinton, Hillary Rodham recebeu a maioria das doacoes quando olhamos em quantidade de doadores, seguida por Sanders, Bernard e Trump, Donald J.
```{r,echo=FALSE}
...
Os candidatos expressivos no estado da California ficam bem visiveis no histograma de doacoes.
```{r, echo=FALSE}
...
As doacoes possuem a mesma distribuicao. A diferenca esta na quantidade de doacoes vista no grafico abaixo que usa alpha = 1/30 para mostrar onde os dados se sobrepoem.
...
#Restringindo os valores segundo a analise anterior que a maioria dos valores se concentram atÈ 1000, mas para evitar jogar dados fora
#vou deixar 2000
top3_candidatos = subset(dados, (cand_nm %in% candidatos) & contb_receipt_amt >0 & contb_receipt_amt <= 1000)
...
graph_plot <-function(profissional){
  ...
  x <- ggplot(data = subset(top3_candidatos, contbr_occupation==profissional), aes(x=contb_receipt_amt))+
...
for(i in 1:10){
  grid.arrange(graph_plot(top_ocupacoes[i]))
}
```
...
#Analise temporal
Contribuicao de aposentados e desempregados no tempo nao apresenta nenhum padrao destacavel, mas o grafico pode ser polido para incrementar as informacoes.
```{r}
...
##Grafico Violino
Fica claro o qu efoi visto anteriormente sobro valor medio das doacoes e a quantidade de doadores.
...
```{r,echo=FALSE}
ggplot(data = subset(top3_candidatos,contbr_occupation %in%  top_ocupacoes), aes(x=cand_nm,y=contb_receipt_amt))+
...
ggplot(data = dados, aes(x = cand_nm, fill = ..count../sum(..count..)))+
  ...
  scale_fill_gradientn(colours = colorRampPalette(c("red","yellow"))(100),guide = guide_legend( title = "Proporcao das doacoes(Quantidade)"))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  xlab("Candidato")+
  ylab("Contagem")+
...
Os valores se espalham de forma aleatoria, mas podemos ver pela concentracao de linhas cinzas no final do lado direito que uma suposicao plausÌvel seria que as doacoes perdem forca no final do periodo considerado na base de dados, ou seja, a maior parte do dinheiro que chega por doacoes vem antes das semanas finais do data set.
```{r,echo=FALSE}
ggplot(data=subset(top3_candidatos, contbr_occupation %in% top_ocupacoes),
       aes(x=contb_receipt_dt,y=contbr_occupation))+
  geom_tile(aes(fill = contb_receipt_amt))+
  scale_fill_gradientn(colours = colorRampPalette(c("red","yellow","blue"))(100),guide = guide_legend( title = "Valor Doacao"),breaks=seq(0,1000,100))+
  scale_x_discrete(breaks = 1:20)+
  xlab("Data")+
  ylab("Ocupacao")+
  ggtitle("Doacao x Ocupacao por dia")+
   theme(axis.text.x=element_text(colour="darkblue"), 
        axis.text.y = element_text(colour = "darkblue"),
        axis.title.x = element_text(colour = "midnightblue"),
        axis.title.y = element_text(colour = "midnightblue"),
